...
While the conceptual architecture describes the simulator's components, the practical implementation is organized within a modular Python package named \texttt{ev2gym}. This structure promotes code reusability and a clear separation of concerns. The high-level experimentation scripts, such as \\
 \noindent 
\texttt{run\_experiments.py} and \texttt{train\_mpc\_approximator.py}, reside in the project's root directory and act as orchestrators, utilizing the core functionalities provided by the \texttt{ev2gym} package.
...
\begin{itemize}
    ...
    \]
    \item \textbf{Prioritized Experience Replay (PER):} This work enhances DDPG with PER. Instead of uniform sampling from the replay buffer $D$, PER samples transitions (MILP) their TD-error, prioritizing those where the model has the most to learn. The probability of sampling transition $i$ is:
    \[
