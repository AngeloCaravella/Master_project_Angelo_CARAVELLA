...
The total reward at each timestep $t$, $R_t$, is formulated as a scaled net economic profit $\Pi_t$ minus any active penalties for user dissatisfaction $P_t^{\text{sat}}$ or transformer overload $P_t^{\text{tr}}$.

\begin{equation}
    R_t = \alpha_{\pi} \Pi_t - P_t^{\text{sat}} - P_t^{\text{tr}}
\end{equation}
...
\begin{itemize}
    ...
    \item[$\alpha_{\pi}$] is a scaling factor for the profit component (set to 0.1 in the implementation).
    ...
    \item[$F_{hist}^{\text{tr}}$] is the frequency of transformer overload events over the same history window.
...
\begin{algorithm}[H]
\caption{History-Based Adaptive Reward Calculation (\texttt{FastProfitAdaptiveReward})}
...
\State Define profit scale $\alpha_{\pi} \leftarrow 0.1$.
\State Define base multipliers $\lambda_{\text{base}}^{\text{sat}} \leftarrow 200.0$, $\lambda_{\text{base}}^{\text{tr}} \leftarrow 50.0$.
\State Define base overload penalty $P_{\text{base}}^{\text{tr}} \leftarrow 5.0$.

\Function{CalculateReward}{profit $\Pi_t$, satisfaction list $S_{\text{current}}$, overload amount $O_t$}
    \State \Comment{1. Calculate Scaled Economic Profit}
    \State $R_t \leftarrow \alpha_{\pi} \cdot \Pi_t$
    
    \State \Comment{2. Calculate Adaptive Satisfaction Penalty}
    \State $\bar{S}_{hist} \leftarrow \text{Average}(H_{sat})$ or 1.0 if empty.
    \State $\lambda_t^{\text{sat}} \leftarrow \lambda_{\text{base}}^{\text{sat}} \cdot (1 - \bar{S}_{hist})^2$
    \If{$S_{\text{current}}$ is not empty}
        \State $S_{\min} \leftarrow \min(S_{\text{current}})$
        \If{$S_{\min} < 0.95$}
    ...
    \If{$O_t > 0$}
        \State $F_{hist}^{\text{tr}} \leftarrow \text{Average}(H_{tr})$ or 0.0 if empty.
        \State $\lambda_t^{\text{tr}} \leftarrow \lambda_{\text{base}}^{\text{tr}} \cdot F_{hist}^{\text{tr}}$
        \State $P_t^{\text{tr}} \leftarrow P_{\text{base}}^{\text{tr}} + \lambda_t^{\text{tr}} \cdot O_t$
        \State $R_t \leftarrow R_t - P_t^{\text{tr}}$
    \EndIf
    
    \State \Comment{4. Update Historical Data}
    \State $\bar{S}_{\text{current}} \leftarrow \text{Average}(S_{\text{current}})$ or 1.0 if empty.
    \State Append $\bar{S}_{\text{current}}$ to $H_{sat}$.
...
The foundation of the reward signal is the direct, instantaneous economic profit, $\Pi_t$, scaled by a factor $\alpha_{\pi}$. This component provides a clear incentive for the agent to exploit market dynamics by encouraging charging during low-price periods and discharging (V2G) during high-price periods. The scaling factor ensures that the profit signal does not completely dominate the penalty signals, maintaining a balance in the learning process.

\subsubsection{Adaptive User Satisfaction Penalty}
The penalty for failing to meet user charging demands, $P_t^{\text{sat}}$, adapts based on the system's recent performance. The environment maintains a short-term memory of user satisfaction scores in a queue, from which an average historical score, $\bar{S}_{hist}$, is calculated. A \textit{satisfaction severity multiplier}, $\lambda_t^{\text{sat}}$, is then computed, which grows quadratically as the historical average satisfaction drops. This ensures that as performance degrades, the consequences of new failures become substantially more severe.
\begin{equation}
    \lambda_t^{\text{sat}} = \lambda_{\text{base}}^{\text{sat}} \cdot (1 - \bar{S}_{hist})^2
\end{equation}
\noindent
where $\lambda_{\text{base}}^{\text{sat}}$ is a base scaling factor (200.0). A penalty is only applied if the minimum satisfaction score among currently connected users, $S_{\min}$, falls below a critical threshold (95\%). The magnitude of the penalty is the product of the adaptive multiplier and the current satisfaction deficit.
\begin{equation}
    P_t^{\text{sat}} = 
    \begin{cases} 
      \lambda_t^{\text{sat}} \cdot (1 - S_{\min}) & \text{if } S_{\min} < 0.95 \\
      0 & \text{otherwise}
    \end{cases}
\end{equation}
\noindent
...
The transformer overload penalty, $P_t^{\text{tr}}$, operates on a similar adaptive principle, responding to the recent frequency of overloads, $F_{hist}^{\text{tr}}$. This frequency is tracked over the same historical window and directly drives the computation of a linear \textit{overload severity multiplier}. If the total power drawn, $O_t$, exceeds the transformer's limit, a penalty is applied. This penalty consists of a fixed base amount $P_{\text{base}}^{\text{tr}}$ plus an adaptive component, which scales with both the historical frequency of overloads and the magnitude of the current overload.
\begin{equation}
    P_t^{\text{tr}} = 
    \begin{cases} 
      P_{\text{base}}^{\text{tr}} + (\lambda_{\text{base}}^{\text{tr}} \cdot F_{hist}^{\text{tr}}) \cdot O_t & \text{if } O_t > 0 \\
      0 & \text{otherwise}
    \end{cases}
\end{equation}
\noindent
where $\lambda_{\text{base}}^{\text{tr}}$ is a base scaling factor (50.0) and $P_{\text{base}}^{\text{tr}}$ is a constant base penalty (5.0). This structure enforces a critical lesson: while an occasional, minor overload might be tolerable in pursuit of high profit, habitual overloading becomes increasingly costly as penalties escalate, forcing the agent to respect physical constraints.
...
\paragraph{Implementation Details}
This reward function is implemented in the \texttt{reward.py} module. The \texttt{FastProfitAdaptiveReward} function uses \texttt{collections.deque} objects, attached to the environment instance at runtime, to efficiently maintain a sliding window of recent performance for both satisfaction and overload events. This method is computationally efficient, avoiding complex state-dependent calculations in favor of simple updates to the historical data queues.