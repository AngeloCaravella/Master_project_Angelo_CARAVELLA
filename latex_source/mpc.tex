...
\paragraph{The Finite-Time Optimal Control Problem}

At each time step $t$, given the current state measurement $x(t)$, the MPC law is defined by solving a constrained finite-time optimal control problem (CFTOC).
...
\paragraph{The Receding Horizon Policy}
...
\paragraph{Summary of the MPC Workflow}
...
\caption{On the left, a convex set; in the middle, a non-convex set; on the right, a convex function.}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Approximating.png} 
...
\subsubsection{MPC: Classifications}
\begin{itemize}
...
\subsection{Improvement of the Standard Fixed-Horizon MPC Formulation}
...
\subsubsection{Limitation of the Fixed-Horizon Approach}
...
\subsubsection{Improving the Formulation with Adaptive Horizon MPC (AHMPC)}
...
\subsubsection{The Ideal (but Impractical) AHMPC Scheme}
...
\subsubsection{The Practical AHMPC Algorithm}
...
\subsubsection{Advantages of the AHMPC Formulation}
...
\subsection{Further Enhancements via Learning-Based Approaches}
