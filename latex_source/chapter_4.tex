...
\end{itemize}
\noindent
The analysis was conducted using the custom-built \texttt{sensitivity\_analysis.py} script,
\\
...
\subsection{Applications and Benefits of SA}
...
\section{What Really Matters?}
An example is provided for understanding the variation of radiation dose over time for individuals near nuclear repositories. Modeling this involves numerous processes like transport, flow, reactions, and geochemistry, resulting in an extremely complicated nonlinear system. The output, however, is a simple function over time. The goal of sensitivity analysis in this context is to understand which of the complex inputs truly matter and what interactions are necessary to comprehend the final performance measure.
...
\subsection{One-at-a-time (OAT)}
One-at-a-time analysis involves taking a base case (e.g., the mean of all parameters), fixing all other parameters, and varying one parameter at a time (e.g., by 10\%) to observe its influence. Simulations are run for these perturbations, and the influence of that parameter is recorded. The parameters can then be ranked from the most to the least influential, often displayed in a tornado chart.

\begin{itemize}
    \item Most widely used screening method due to its simplicity.
    \item Varies the input parameters one by one, keeping the other inputs fixed at a baseline \& monitors changes in the output.
    \item Results often displayed in a Pareto chart (requires $2 \times N_p + 1$ simulations).
    \item Similar to a local method.
\end{itemize}

A problem with this method is that by fixing parameters, interactions are not studied. Also, the assumption that the base case (average of parameters) gives an average response is not true for non-linear systems. It also does not handle discrete parameters well.

\subsection{The Morris Method}
The Morris method is an extension of the one-at-a-time (OAT) designs. Instead of a single base case, a randomization is performed over $L$ trajectories (or base cases).
\begin{itemize}
    \item Based on a replication of OAT designs.
    \item Based on the calculation of elementary effects ($EE_n$).
    \item Evaluated by perturbing the n-th parameter of a point $x = (x_1, \dots, x_{N_p})$ by $\Delta$.
    
    The elementary effect is calculated as:
    \[ EE_n = \frac{f(x_1, \dots, x_n + \Delta, \dots, x_{N_p}) - f(x_1, \dots, x_n, \dots, x_{N_p})}{\Delta} \]
    \item To avoid dependency on the initial point $x$, the $EE_n$ are computed for $L$ trajectories, which results in a distribution of $EE_n$.
\end{itemize}

From this distribution, the mean and variance of the elementary effects can be calculated.

\begin{itemize}
    \item The mean of the absolute values of the elementary effects, $\mu_n^*$, assesses the overall impact (influence) of the parameter on the response.
    \[ \mu_n^* = \frac{1}{L} \sum_{l=1}^{L} |EE_n^{(l)}| \]
    A large mean indicates large changes in the response when perturbing a parameter (an influential parameter).
    
    \item The standard deviation or variance, $\sigma_n^2$, is a measure of non-linear and/or interaction effects.
    \[ \sigma_n^2 = \frac{1}{L-1} \sum_{l=1}^{L} (EE_n^{(l)} - \mu_n)^2 \]
    A large value indicates that the elementary effects depend highly on the choice of the sample point at which it is computed. A small $\sigma_n^2$ indicates similar values in elementary effects for different sample points, implying that the effect of the parameter is independent of other parameter values.
\end{itemize}

By plotting $\sigma_n$ versus $\mu_n^*$, parameters can be classified into three groups:
\begin{enumerate}
    \item \textbf{Negligible effects:} small $\mu_n^*$ and small $\sigma_n$.
    \item \textbf{Linear and additive effects:} large $\mu_n^*$ and small $\sigma_n$.
    \item \textbf{Non-linear and/or interactions effects:} large $\mu_n^*$ and large $\sigma_n$.
\end{enumerate}
...
\subsection{Summary of Screening Techniques}
\begin{itemize}
...
\noindent
To systematically investigate the V2G system's behavior and train robust control agents, this work employs a suite of increasingly sophisticated methodologies, all orchestrated through the \texttt{sensitivity\_analysis.py} and \texttt{run\_interactive.py} scripts. These methods range from classical sensitivity analysis techniques to advanced, dynamic training environments.
...
To conduct the global sensitivity analysis using the Morris method, the Python library \texttt{SALib} was utilized. The process implemented in the \texttt{sensitivity\_analysis.py} script can be broken down into the following key steps:

\begin{enumerate}
    \item \textbf{Problem Definition:}
    First, the sensitivity problem is defined. This is done by creating a Python dictionary that specifies the parameters to be analyzed, their names, and their bounds. This data structure, named \texttt{problem}, is a fundamental requirement for the \texttt{SALib} library.
...
problem = {
    ...
    \item \textbf{Sampling:}
    Next, the Morris sampler provided by \texttt{SALib} is used to generate the input points to be evaluated. The function \texttt{morris\_sampler.sample()} takes the \texttt{problem} dictionary, the number of trajectories to generate (\texttt{N}), and the number of grid levels (\texttt{num\_levels}) as input. The function returns a NumPy array, \texttt{param\_values}, where each row represents a combination of input parameters on which to run the model.
...
param_values = morris_sampler.sample(problem, 
    ...
    The code then iterates over each row of the \texttt{param\_values} array. For each combination of parameters, a model simulation is executed (\texttt{run\_benchmark}), and the output of interest (e.g., \texttt{total\_profits}) is recorded. These results are stored in a NumPy array \texttt{Y}.

    \item \textbf{Analysis of Results:}
    Once the model has been run for all sampling points, the results are analyzed using the \texttt{morris\_analyzer.analyze()} function. This function calculates the Morris sensitivity indices, primarily:
    \begin{itemize}
...
Si = morris_analyzer.analyze(problem, 
    ...
    The returned \texttt{Si} object contains the results of the analysis, which are then used to generate tables and plots to interpret the influence of each parameter.
...
\begin{itemize}
    ...
    \item \textbf{Region 2: High $\mu^*$, High $\sigma$ (Strong Influence, Non-linear/Interactive)}: These are the most critical parameters. They have a significant overall impact, but their effect is highly dependent on the values of other parameters. This indicates strong non-linearities or interactions. \texttt{Discharge Price Factor} and \texttt{Transformer Max Power} often fall into this category in V2G systems, as their impact is heavily modulated by other conditions.
    \item \textbf{Region 3: Low $\mu^*$, Low $\sigma$ (Negligible Influence)}: Parameters located here have little to no effect on the model output. They are generally unimportant and can often be fixed without significantly altering the model's behavior. \texttt{EV Charge Efficiency} might be an example if its variation range is small compared to other factors.
...
\noindent
This method uses a custom Gymnasium environment, \texttt{RobustnessEnv}, which wraps the core \texttt{EV2Gym} simulator. At the beginning of every single training episode, when the \texttt{reset()} function is called, this wrapper performs the following actions in memory:
\begin{enumerate}
    ...
    \item It initializes a new instance of the underlying \texttt{EV2Gym} environment for that episode using this dynamic configuration.
\end{enumerate}
\noindent
...
\subsubsection{Analysis on the Number of Charging Stations}
In this analysis, we vary the number of charging stations to observe its effect on system performance. The results are presented in the figures below.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_average_transformer_loading_pct.png}
    \caption{Sensitivity of Average Transformer Loading Pct to the Number of Charging Stations.}
    \label{fig:sa_ncs_average_transformer_loading_pct}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_average_user_satisfaction.png}
    \caption{Sensitivity of Average User Satisfaction to the Number of Charging Stations.}
    \label{fig:sa_ncs_average_user_satisfaction}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_battery_degradation_calendar.png}
    \caption{Sensitivity of Battery Degradation Calendar to the Number of Charging Stations.}
    \label{fig:sa_ncs_battery_degradation_calendar}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_battery_degradation_cycling.png}
    \caption{Sensitivity of Battery Degradation Cycling to the Number of Charging Stations.}
    \label{fig:sa_ncs_battery_degradation_cycling}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_battery_degradation.png}
    \caption{Sensitivity of Battery Degradation to the Number of Charging Stations.}
    \label{fig:sa_ncs_battery_degradation}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_calendar_degradation.png}
    \caption{Sensitivity of Calendar Degradation to the Number of Charging Stations.}
    \label{fig:sa_ncs_calendar_degradation}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_cyclic_degradation.png}
    \caption{Sensitivity of Cyclic Degradation to the Number of Charging Stations.}
    \label{fig:sa_ncs_cyclic_degradation}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_energy_tracking_error.png}
    \caption{Sensitivity of Energy Tracking Error to the Number of Charging Stations.}
    \label{fig:sa_ncs_energy_tracking_error}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_energy_user_satisfaction.png}
    \caption{Sensitivity of Energy User Satisfaction to the Number of Charging Stations.}
    \label{fig:sa_ncs_energy_user_satisfaction}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_execution_time.png}
    \caption{Sensitivity of Execution Time to the Number of Charging Stations.}
    \label{fig:sa_ncs_execution_time}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_min_energy_user_satisfaction.png}
    \caption{Sensitivity of Min Energy User Satisfaction to the Number of Charging Stations.}
    \label{fig:sa_ncs_min_energy_user_satisfaction}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_peak_transformer_loading_pct.png}
    \caption{Sensitivity of Peak Transformer Loading Pct to the Number of Charging Stations.}
    \label{fig:sa_ncs_peak_transformer_loading_pct}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_power_tracker_violation.png}
    \caption{Sensitivity of Power Tracker Violation to the Number of Charging Stations.}
    \label{fig:sa_ncs_power_tracker_violation}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_profit_per_ev.png}
    \caption{Sensitivity of Profit Per Ev to the Number of Charging Stations.}
    \label{fig:sa_ncs_profit_per_ev}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_std_energy_user_satisfaction.png}
    \caption{Sensitivity of Std Energy User Satisfaction to the Number of Charging Stations.}
    \label{fig:sa_ncs_std_energy_user_satisfaction}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_total_energy_charged.png}
    \caption{Sensitivity of Total Energy Charged to the Number of Charging Stations.}
    \label{fig:sa_ncs_total_energy_charged}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_total_energy_discharged.png}
    \caption{Sensitivity of Total Energy Discharged to the Number of Charging Stations.}
    \label{fig:sa_ncs_total_energy_discharged}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_total_profits.png}
    \caption{Sensitivity of Total Profits to the Number of Charging Stations.}
    \label{fig:sa_ncs_total_profits}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_total_reward.png}
    \caption{Sensitivity of Total Reward to the Number of Charging Stations.}
    \label{fig:sa_ncs_total_reward}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_total_steps_min_emergency_battery_capacity_violation.png}
    \caption{Sensitivity of Total Steps Min Emergency Battery Capacity Violation to the Number of Charging Stations.}
    \label{fig:sa_ncs_total_steps_min_emergency_battery_capacity_violation}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_tracking_error.png}
    \caption{Sensitivity of Tracking Error to the Number of Charging Stations.}
    \label{fig:sa_ncs_tracking_error}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/sensitivity_Number_of_Charging_Stations_20251012_110213/sensitivity_Number_of_Charging_Stations_vs_transformer_overload_kwh.png}
    \caption{Sensitivity of Transformer Overload Kwh to the Number of Charging Stations.}
    \label{fig:sa_ncs_transformer_overload_kwh}
\end{figure}

\subsection{Morris Method Global Analysis Results}
The Morris method provides a higher-level view of the entire system's sensitivity. By analyzing the influence of multiple parameter changes simultaneously, it helps to identify not only the most influential parameters but also those that have strong non-linear effects or interactions. The following figures present the \(\mu^*-\sigma\) plots for each of the tested control algorithms.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/AFAP/morris_analysis_plot_AFAP.png}
    \caption{Morris method sensitivity plot for the AFAP algorithm.}
    \label{fig:morris_afap}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/ALAP/morris_analysis_plot_ALAP.png}
    \caption{Morris method sensitivity plot for the ALAP algorithm.}
    \label{fig:morris_alap}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/DDPG/morris_analysis_plot_DDPG.png}
    \caption{Morris method sensitivity plot for the DDPG algorithm.}
    \label{fig:morris_ddpg}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/DDPG+PER/morris_analysis_plot_DDPG+PER.png}
    \caption{Morris method sensitivity plot for the DDPG+PER algorithm.}
    \label{fig:morris_ddpg_per}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/Online_MPC_Adaptive/morris_analysis_plot_Online_MPC_Adaptive.png}
    \caption{Morris method sensitivity plot for the Online MPC Adaptive algorithm.}
    \label{fig:morris_online_mpc_adaptive}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/Online_MPC_NonAdaptive/morris_analysis_plot_Online_MPC_NonAdaptive.png}
    \caption{Morris method sensitivity plot for the Online MPC NonAdaptive algorithm.}
    \label{fig:morris_online_mpc_nonadaptive}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/RR/morris_analysis_plot_RR.png}
    \caption{Morris method sensitivity plot for the RR algorithm.}
    \label{fig:morris_rr}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/SAC/morris_analysis_plot_SAC.png}
    \caption{Morris method sensitivity plot for the SAC algorithm.}
    \label{fig:morris_sac}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../sensitivity_analysis_results/morris_analysis_20251013_093108/TQC/morris_analysis_plot_TQC.png}
    \caption{Morris method sensitivity plot for the TQC algorithm.}
    \label{fig:morris_tqc}
\end{figure}

Based on this analysis, we can typically categorize the parameters:
\begin{itemize}
    \item \textbf{Highly Influential:} Parameters in the top-right quadrant (high \(\mu^*\), high \(\sigma\)) are the most critical. These would likely include \texttt{Discharge Price Factor} and \texttt{Transformer Max Power}, as they fundamentally define the economic incentives and physical limits of the system. Their high \(\sigma\) indicates that their effect is highly dependent on other system conditions.
    \item \textbf{Influential with Low Interaction:} Parameters with high \(\mu^*\) but low \(\sigma\) have a strong, mostly linear effect on the output. \texttt{EV Spawn Multiplier} might fall into this category, as increasing the number of EVs generally increases total potential profit, but in a relatively predictable way.
    \item \textbf{Non-Influential or Redundant:} Parameters in the bottom-left (low \(\mu^*\), low \(\sigma\)) have little impact on the output metric. For example, \texttt{EV Charge Efficiency} might be in this group if its range of variation (0.80-0.99) does not cause a significant enough change in total energy cost to affect the overall profit substantially compared to market price fluctuations.
\end{itemize}

The Morris analysis is invaluable for model simplification and for focusing control strategy development. It tells us which parameters must be modeled with high fidelity and which ones are less critical.

\section{Conclusion}
The sensitivity analysis performed in this chapter provides critical insights into the dynamics of the V2G control problem. The results clearly indicate that the system's performance is not equally sensitive to all parameters. Economic factors, such as the \texttt{Discharge Price Factor}, and hard physical constraints, like the \texttt{Transformer Max Power}, were identified as the most dominant drivers of profitability and grid stability.
\noindent
