...
class OnlineMPC_Solver:
    ...
    def __init__(self, env, prediction_horizon=10, control_horizon='half',
                 ...
                 solver_timeout=50, # New parameter for solver timeout in seconds
                 **kwargs):
        self.env = env
        ...
        self.solver_timeout = solver_timeout # Store the timeout
    ...
    def get_action(self, env):
        ...
        try:
            ...
            prob.solve(pulp.PULP_CBC_CMD(msg=0, timeLimit=self.solver_timeout))
            end_solve_time = time.time()
            solve_duration = end_solve_time - start_solve_time
            if solve_duration > self.solver_timeout:
                print(f"--- MPC WARNING (Strict): Solver timed out after {self.solver_timeout} seconds (Step: {current_step}) ---")
                status = 'Timeout'
            else:
                status = pulp.LpStatus[prob.status]
        except Exception as e:
            print(f"--- MPC ERROR (Strict): Solver encountered an exception: {e} (Step: {current_step}) ---")
            status = 'Error'
        ...
        if self.use_adaptive_horizon:
            if status == 'Optimal':
                ...
                # Solver failed or timed out, increase horizon as a safeguard
                self.current_H = min(self.h_max, self.current_H + 1)
        ...
        if status == 'Optimal':
            ...
            return np.clip(action, -1, 1), status
        
        elif status == 'Infeasible':
            pass # Status is captured, no need to print at runtime
        elif status == 'Timeout':
            pass # Status is captured, no need to print at runtime
        else:
            pass # Status is captured, no need to print at runtime

        return np.zeros(num_cs), status
